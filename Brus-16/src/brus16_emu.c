...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char **argv) {
    ...
    *appstate = malloc(sizeof(struct EMU));
    ...
    assert(SDL_CreateWindowAndRenderer("Brus-16", 0, 0, 0, &emu->window, &emu->renderer));
    SDL_SetRenderLogicalPresentation(emu->renderer, SCREEN_W * ZOOM, SCREEN_H * ZOOM, SDL_LOGICAL_PRESENTATION_LETTERBOX);
...
SDL_AppResult SDL_AppIterate(void *appstate) {
    ...
    while (emu->ticks_acc >= SDL_NS_PER_SECOND) {
        ...
        for (int i = 0; i < SAMPLES_PER_FRAME; i++) {
            int16_t sample = sfx_process(&emu->sfx);
            samples[i] = sample;
            fprintf(fp, "%d\n", sample);
        }
